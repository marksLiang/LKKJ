...
final class   CommonFunction {
    ...
    ///导航栏的高度
    static let NavigationControllerHeight = CommonFunction.StatusBarHeight+44
    /// 屏幕的宽度
    ...
    static let IOS11 = (UIDevice.current.systemVersion as NSString).doubleValue >= 11.0
    ...
    static func SystemColor()->UIColor{
        return  UIColor().TransferStringToColor("#1CADE4")
    }
    ...
    static func Global_Get<T:NSObject>(
        ...
        AFNHelper.get(nil, urlString: url, parameters:parameters,isHUD: isHUD,HUDMsg:HUDMsg,isHUDMake: isHUDMake, success: { (json) in
            ...
            if(json != nil){    //==nil表示失败的
                if(Model != nil){
                    ...
                    if(value.data  != nil ){
                        if(entity != nil){
                            if(IsListData==true){
                                let model = T.mj_objectArray(withKeyValuesArray: value.data)
                                value.data=model
                            }else{
                                let model = T.mj_object(withKeyValues: value.data)
                                value.data=model 
                            }
                        }
                    }
            ...
            if(Model != nil){
                ...
                resulet.msg="错误编码:\(error.code) ，"+error.domain
                resulet.status=0
                Model?(resulet)
            }
            print(error)
