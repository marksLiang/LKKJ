...
class MyHeadUIView: UIView {
    ...
    func _layer(tableHeaderView:UITableView,target:UIViewController,selector:Selector) -> UIView{
        self.frame=CGRect(x: CGFloat(0), y: CGFloat(0), width: CGFloat(UIScreen.main.bounds.size.width), height: CGFloat(200))
        self.backgroundColor = UIColor.white
        tableHeaderView.tableHeaderView = self
        let layer2 = CAShapeLayer()
        layer2.fillColor = CommonFunction.SystemColor().cgColor
        ...
        bgImage = UIImageView.init(frame: self.frame)
        bgImage.image = UIImage.init(named: "background_01")
