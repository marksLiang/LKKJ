...
class registerViewController: UIViewController,UITableViewDelegate,UITableViewDataSource
{
    ...
    lazy var backbtn:UIButton =
        {
            let btn = UIButton(frame: CGRect(x: 0, y: 22, width: 50, height: 43))
    ...
    lazy var registerbtn:UIButton =
        {
            let btn = UIButton(frame: CGRect(x: 20, y: 25, width: self.view.bounds.width-40, height: 35))
            btn.backgroundColor=CommonFunction.SystemColor()
            btn.setTitleColor(UIColor.white, for: .normal)
            btn.setTitle(  "确认注册",   for: .normal)
    ...
    lazy var backgoundImage: UIImageView = {
        let backgoundImage = UIImageView.init(frame: self.view.bounds)
        backgoundImage.image = UIImage(named: "background_01.png")
//        backgoundImage.backgroundColor = UIColor.red
        return backgoundImage
    }()

        typealias CallbackValue=(_ phone:String )->Void //类似于OC中的typedef
    ...
    override func viewDidLoad()
    {
        ...
        self.view.addSubview(backgoundImage)//添加背景图
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        case 1:
            ...
            cell.inpuText.isSecureTextEntry=true
            cell.inpuText.rx.text.orEmpty
        ...
        case 2:
            cell.lab.text="确认密码:"
            cell.inpuText.placeholder="请再次输入密码"
            cell.inpuText.isSecureTextEntry=true
            cell.inpuText.rx.text.orEmpty
                .bind(to: _regiestViewModel.againPassword) //绑定密码
                .addDisposableTo(disposeBag)
            break
        case 3:
            cell.lab.text="验证码:"
    ...
    func RegisterResult(){
        ...
        _ = self._regiestViewModel.registeResult?.subscribe(onNext: { (result) in
            ...
            case   .ok: //处理成功的业务
                ...
                self._regiestViewModel.SetRegister(result: { (isRes,phone) in
                    if isRes! {
                        if self.myCallbackValue != nil {
                            self.myCallbackValue!(phone)
                        }
