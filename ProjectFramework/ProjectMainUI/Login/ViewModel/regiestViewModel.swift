...
//                }
    ...
    ///注册
    func SetRegister(result:((_ result:Bool?, _ phone:String) -> Void)?){
        let parameters = ["username":username.value,"password":password.value,"password2":againPassword.value]
        CommonFunction.Global_Post(entity: nil, IsListData: false, url:  HttpsUrl+"index.php/Reg/save", isHUD: true,HUDMsg: "正在提交中...", isHUDMake: false, parameters:parameters as NSDictionary) { (resultData) in
//            print(resultData?.ret,resultData?.Content)
            //注册
            if(resultData?.status==200){
                if (resultData?.data != nil){
                    let dic = resultData?.data as! Dictionary<String, Any>
                    let model = LoginMode.mj_object(withKeyValues: dic["register"])
                    result?(true,model!.username)
                }
            }else{
