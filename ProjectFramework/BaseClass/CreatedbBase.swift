...
class CreatedbBase {
    ...
    func CreateDB( ){
        ...
        var DBItem=[String]()
        //用户信息
        let createSql1:String = "CREATE TABLE IF NOT EXISTS MemberInfo (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, userid text , phone text , nickname text ,  sex text , ImagePath text , token text  , IsLogin bit )"
    ...
    func DefaultDBData(){
        ...
        CommonFunction.ExecuteQuery("select *  from MemberInfo", nil) { (Result) in
            ...
            if(!Result.next()){
                CommonFunction.ExecuteUpdate("insert into MemberInfo (userid,phone,nickname,sex,ImagePath,token,IsLogin ) values (?,?,?,?,?,?,? ) ", ["0" as AnyObject,"" as AnyObject,"" as AnyObject,"" as AnyObject,"" as AnyObject,"" as AnyObject,false as AnyObject]) { (isOk) in
                }
            }else{
                CommonFunction.ExecuteQuery("select * from MemberInfo", nil, callback: { (Result) in
                    while Result.next() {
                        print(Result.resultDictionary())
                        let userid = Result.string(forColumn: "userid") as String
                        let PhoneNo = Result.string(forColumn: "phone") as String
                        let RealName = Result.string(forColumn: "nickname") as String
                        ...
                        Global_UserInfo.userid=userid
                        Global_UserInfo.phone=PhoneNo
                        Global_UserInfo.nickname=RealName
